...
int crash_shrink_memory(unsigned long new_size)
{
    ...
    typedef int insert_resource_f(struct resource *parent, struct resource *new);
    insert_resource_f * insert_resource = (insert_resource_f *) 0xc003c31c;
...
int kernel_kexec(void)
{
...
#endif
	{
		...
		printk(KERN_INFO "Starting new kernel\n");
		machine_shutdown();
	}
...
static int __init kexec_module(void) {
    ...
    printk(KERN_DEBUG "kexec_load installed\n");
    ret = crash_notes_memory_init();

    if (ret) return ret;

    ret = crash_save_vmcoreinfo_init();
